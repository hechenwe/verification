...
public class ApiDocAop {
			...
			for (InterfacModel im : mm.getInterfacModels()) {
				...
				for (ArrayModel am : im.getArrayModels()) {
					...
					for (ParameterModel pm4ArrayModel : pmes4ArrayModel) {
						...
						if(am.getIsMust().equals("false")) {
							pm4ArrayModel.setIsMust("false");
				...
				for (ObjectModel om : im.getObjectModels()) {

					ParameterModel pm = new ParameterModel();
					pm.setInterfacId(interfacId);
					pm.setIsMust(om.getIsMust());
					pm.setParameterCode(om.getKey());
					pm.setParameterName(om.getChineseAnnotation());
					pm.setMaxLength(Integer.MAX_VALUE);
					pm.setMinLength(0);
					pm.setParameterDataType("JsonObject");
					pm.setParameterExample("...");

					pmes.add(pm);
					List<ParameterModel> pmes4ObjectModel = om.getParameterModels();

					for (ParameterModel pm4ArrayModel : pmes4ObjectModel) {
						pm4ArrayModel.setInterfacId(interfacId);
						if(om.getIsMust().equals("false")) {
							pm4ArrayModel.setIsMust("false");
						}
					}
					pmes.addAll(pmes4ObjectModel);
				}
	...
	public Map<String, Object> getObject4Json(List<ObjectModel> objectModels) {
		Map<String, Object> map = new LinkedHashMap<>();
		for (ObjectModel objectModel : objectModels) {
			Map<String, Object> objectMap = getParameter4Json(objectModel.getParameterModels());
			Map<String, Object> m = getArray4Json(objectModel.getArrayModels());
			objectMap.putAll(m);
			map.put(objectModel.getKey(), objectMap);
	...
	public Map<String, Object> getMap4Json4Interfac(InterfacModel interfacModel) {
		...
		map.putAll(getObject4Json(interfacModel.getObjectModels()));
		return map;
